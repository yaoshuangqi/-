...
<div class="check-div">
    <div class="col-xs-4">
    <button class="btn btn-yellow btn-xs" data-toggle="modal" data-target="#addChar">添加标签</button>
        </div>
</div>
...
<script>
    ...
    $("#add").click(function () {
        $.ajax({
            type:"post",
            url:"<%=application.getContextPath()%>/tag/add",
            dataType: 'json'
            ,data:{name:$("#name").val()}
    ...
    $(".del").click(function () {
        if(confirm("是否删除?")){
            $.ajax({
                type:"get",
                url:"<%=application.getContextPath()%>/tag/del",
                dataType: 'json'
